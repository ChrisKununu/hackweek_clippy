...
def main():
    ...
    pipe_blog = data_setup(credentials, config, index_name='index_name')
    pipe_reviews = data_setup(credentials, config, index_name='review_index')
    ...
    blog, review = st.tabs(["blog insights", "FERCHAU review QA"])
        ...
        *Note: do not use keywords, but full-fledged questions.* The demo is not optimized to deal with keyword queries and might misunderstand you.
        ...
        if search_term:
            with st.spinner(
                ...
                blog_prediction = pipe_blog.run(
                    query=search_term.lower(),
                    params={"Retriever": {"top_k": top_k_retriever}, "Reader": {"top_k": top_k_reader}}
                    )

                for i in range(len(blog_prediction['answers'])):
                    answer = blog_prediction['answers'][i].answer
                    context = blog_prediction['answers'][i].context
                    start_idx = context.find(answer)
                    end_idx = start_idx + len(answer)
                    score = round(blog_prediction['answers'][i].score * 100, 2)
                    document_id = blog_prediction['answers'][i].document_id
                    doc = [x for x in blog_prediction['documents'] if x.id == document_id][0]
                    meta = doc.meta
    ...
    with review:
        with st.spinner(
            ...
            # Title
            st.write("## find out about Ferchau as an Employer")
            st.write('[Profile Link](https://www.kununu.com/de/ferchau)')
            st.markdown(
                """
        This demo takes its data from Employer reviews about Ferchau.
        ...
        if search_term:
            with st.spinner(
                ...
                for i in range(len(prediction['answers'])):
                    ...
                    meta = doc.meta
                    url, title = meta['review_url'], meta['category']
                    source = f"[{title}]({url})"
                    ...
                    if score >= float(min_score):
                        ...
                        st.markdown(f"**Relevance:** {score} -  **Source:** {url}")
                    #else:
                    #    st.info(
                    #        "🤔 &nbsp;&nbsp; Clippy is unsure whether any of the documents contain an answer to your question. Try to reformulate it!"
                    #        )
